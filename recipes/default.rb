...
adminobj = begin
             if node['rundeck']['admin']['encrypted_data_bag']
             ...
             { 'username' => node['rundeck']['admin']['username'], 'password' => node['rundeck']['admin']['password'], 'ssh_key' => node['rundeck']['admin']['ssh_key'] }
           end
...
recipients = if Chef::Config[:solo]
               ...
               search(node['rundeck']['mail']['recipients_data_bag'], node['rundeck']['mail']['recipients_query'])
                 .map { |_u| eval("u#{node['rundeck']['mail']['recipients_field']}") }
                 .join(',')
             end
...
template '/etc/rundeck/framework.properties' do
  ...
  group 'rundeck'
  mode 00644
  variables(admin_user: adminobj['username'],
...
logrotate_app 'rundeck' do
  ...
  options %w(missingok delaycompress notifempty compress sharedscripts copytruncate)
end
