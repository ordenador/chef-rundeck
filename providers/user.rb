...
action :create do
  ...
  if ::File.read(::File.join('etc', 'rundeck', 'realm.properties')) =~ /^#{new_resource.name}: /
    ...
    ruby_block "rundeck-remove-user-#{new_resource.name}" do
      block do
        ...
        ::File.open(::File.join('etc', 'rundeck', 'realm.properties'), 'r').each do |line|
          newcontent += line unless line =~ /^#{new_resource.name}: /
        end
...
action :update do
  ...
  if ::File.read(::File.join('etc', 'rundeck', 'realm.properties')) =~ /^#{new_auth_line}/
    new_resource.updated_by_last_action(false)
    ...
    ruby_block "rundeck-update-user-#{new_resource.name}" do
      block do
        ...
        ::File.open(::File.join('etc', 'rundeck', 'realm.properties'), 'r').each do |line|
          newcontent += if line =~ /^#{new_resource.name}: /
                          "#{new_auth_line}\n"
                        else
                          line
                        end
